...
int EPET::SetUp( PP &pp, MSK &msk)
{
    ...
    pp.urs_pub.X_=pfc->mult(g,msk.urs_pri.x);
    ...
    for(int i=0;i<ATTRIBUTES_NUM;i++)
    {
        ...
        {
            pp.urs_pub.Z_[i][j]=pfc->mult(g,msk.urs_pri.y[i]);
    ...
    pp.sps_pub.D=pfc->mult(g_,msk.sps_pri.d);
    for(int i=0;i<4;i++)
    {
        pfc->random(msk.sps_pri.e[i]);
        pp.sps_pub.E[i]=pfc->mult(g_,msk.sps_pri.e[i]);
    }
...
int EPET::UserKG(USER_KEY &user_key)
{
    ...
    user_key.upk=pfc->mult(g_,user_key.usk);
    return ret;
}
int EPET::SellerKG(SELLER_KEY &seller_key)
{
    ...
    seller_key.pub_key.X_=pfc->mult(g,seller_key.pri_key.x);
    ...
    seller_key.pub_key.Z02_=seller_key.pub_key.Y[2];
    seller_key.pub_key.Z12_=seller_key.pub_key.Y[2];
    seller_key.pub_key.Z32_=seller_key.pub_key.Y[2];
...
int EPET::IssueCred_u(G2 &upk, USER_ATTR &attr, POK1 &pok1, MSK &msk, Big &VP_u, CRED_U &cred_u)
{
    int ret=0;
    G2 A1,A2,A3;
    //CA verify pok1
    A1=pfc->mult(g_,pok1.s1);
...
{
    int ret=0;
    G1 A1,SUM=pp.urs_pub.X_;
    A1=pfc->mult(pp.urs_pub.Y[0],usk);
    SUM=SUM+A1;
    A1=pfc->mult(pp.urs_pub.Y[ATTRIBUTES_NUM-1],VP_u);
    SUM=SUM+A1;
    for(int i=1;i<ATTRIBUTES_NUM-1;i++)
    {
        A1=pfc->mult(pp.urs_pub.Y[i],attr.Attr[i]);
        SUM=SUM+A1;
    }
    GT E1,E2;
    E1=pfc->pairing(cred_u.sigma2,g);
...
int EPET::ObtainCred_s_Send(SELLER_KEY &seller_key,POK2 &pok2)
{
    ...
    {
        pfc->random(beta[i]);
        P_[i]=pfc->mult(g,beta[i]);
    }
    pfc->start_hash();
    pfc->add_to_hash(seller_key.pub_key.X_);
    for(int i=0;i<4;i++)
    {
        pfc->add_to_hash(seller_key.pub_key.Y[i]);
    }
    for(int i=0;i<5;i++)
    {
...
int EPET::IssueCred_s(SELLER_PUB_KEY &seller_pub,POK2 &pok2,PP &pp, MSK &msk, CRED_S &cred_s)
{
    ...
    G1 A1,A2,R[5];
    //verify pok2
    A1=pfc->mult(g,pok2.t[0]);
    A2=pfc->mult(seller_pub.X_,pok2.c2);
    R[0]=A1+A2;
    for(int i=1;i<5;i++)
    {
        A1=pfc->mult(g,pok2.t[i]);
        A2=pfc->mult(seller_pub.Y[i-1],pok2.c2);
        R[i]=A1+A2;
    }
    pfc->start_hash();
    ...
    for(int i=0;i<4;i++)
    {
        pfc->add_to_hash(seller_pub.Y[i]);
    }
    for(int i=0;i<5;i++)
    {
        pfc->add_to_hash(R[i]);
    ...
    Big rs;
    pfc->random(rs);
    cred_s.A=pfc->mult(g_,pfc->Zpinverse(rs));
    A1=pfc->mult(g,msk.sps_pri.d);
    A2=A1+pp.sps_pub.h_;
    ...
    for(int i=0;i<4;i++)
    {
        A1=pfc->mult(seller_pub.Y[i],msk.sps_pri.e[i]);
...
int EPET::ObtainCred_s_Receive(SELLER_PUB_KEY &seller_pub,CRED_S &cred_s,PP &pp)
{
    int ret=0;
    GT A1,A2,A3,MUL;
    A1=pfc->pairing(cred_s.A,cred_s.C_);
    A2=pfc->pairing(pp.sps_pub.D,pp.sps_pub.h_);
    A3=pfc->pairing(g_,seller_pub.X_);
    MUL=A2*A3;
    for(int i=0;i<4;i++)
    {
        A2=pfc->pairing(pp.sps_pub.E[i],seller_pub.Y[i]);
        MUL=MUL*A2;
    }
    if(MUL != A1) return 1;
    A1=pfc->pairing(cred_s.A,cred_s.B_);
    A2=pfc->pairing(g_,pp.sps_pub.h_);
    A3=pfc->pairing(pp.sps_pub.D,g);
    MUL=A2*A3;
    if(MUL != A1) return 2;
    return ret;
}
int EPET::VerifySellerCred_s(SELLER_PUB_KEY &seller_pub,CRED_S &cred_s,PP &pp)
{
    ...
    A2=pfc->pairing(g_,seller_pub.Z02_);
    if(A1 != A2) return 3;

    A1=pfc->pairing(seller_pub.Y_[1],seller_pub.Y[2]);
    A2=pfc->pairing(g_,seller_pub.Z12_);
    if(A1 != A2) return 4;

    A1=pfc->pairing(seller_pub.Y_[3],seller_pub.Y[2]);
    A2=pfc->pairing(g_,seller_pub.Z32_);
    if(A1 != A2) return 5;

    return 0;
}
int EPET::IssuerVerify(PP &pp,SELLER_KEY &seller_key, CRED_S &cred_s,POK2 &pok2,Big &nounce)
{
    int ret = ObtainCred_s_Send(seller_key,pok2);
    if(ret != 0) return 1;

    pfc->random(nounce);

}
int EPET::ObtainTick_Send(PP &pp, SELLER_PUB_KEY &seller_pub,CRED_S &cred_s, POK2 &pok2, Big &nounce, USER_KEY &user_key, CRED_U &cred_u, USER_ATTR &attr, Big &VP_u, TICK_PRI &tick_pri, USER_PURCH_INFO &purch_info)
{
    //
    G1 A1,A2,R[5];
    //verify pok2
    ...
    for(int i=0;i<DISCLOSE_NUM;i++)
    {
        ...
        purch_info.derive.sigma2_=purch_info.derive.sigma2_+pp.urs_pub.Y[ATTRIBUTES_NUM-DISCLOSE_NUM+i];
    }
    ...
    for(int j=1;j<ATTRIBUTES_NUM-DISCLOSE_NUM;j++)
    {
        A1=pp.urs_pub.Z_[ATTRIBUTES_NUM-DISCLOSE_NUM][j];
        for(int i=ATTRIBUTES_NUM-DISCLOSE_NUM+1;i<ATTRIBUTES_NUM;i++)
        {
            A1=A1+pp.urs_pub.Z_[i][j];

        }
        A1=pfc->mult(A1,attr.Attr[j]);
        purch_info.derive.sigma2_=purch_info.derive.sigma2_+A1;
    ...
    G1 B2;
    B2=pfc->mult(pp.urs_pub.Y[0],k);
    purch_info.derive.sigma3=pfc->pairing(purch_info.derive.sigma1,B2);
    ...
    purch_info.derive.s=pfc->Zpadd(k,temp);
        //PSu
    B2=pfc->mult(pp.urs_pub.Y[0],user_key.usk);
    GT D=pfc->pairing(purch_info.derive.sigma1,B2);
    pfc->random(tick_pri.dsid);
    pfc->random(tick_pri.dsrnd);
    purch_info.Psu=pfc->mult(seller_pub.Y_[0],user_key.usk);
    B1=pfc->mult(seller_pub.Y_[1],tick_pri.dsid);
    purch_info.Psu=purch_info.Psu+B1;
    B1=pfc->mult(seller_pub.Y_[2],tick_pri.dsrnd);
    purch_info.Psu=purch_info.Psu+B1;
    ...
    GT RD;
    G2 RPsu;
    B2=pfc->mult(pp.urs_pub.Y[0],alfau);
    RD=pfc->pairing(purch_info.derive.sigma1,B2);

    RPsu=pfc->mult(seller_pub.Y_[0],alfau);
    B1=pfc->mult(seller_pub.Y_[1],alfad);
    RPsu=RPsu+B1;
    B1=pfc->mult(seller_pub.Y_[2],alfar);
    RPsu=RPsu+B1;
...
int EPET::IssueTick(PP &pp,USER_PURCH_INFO &purch_info,SELLER_KEY &seller_key,TICKET &tick,Big &nounce)
{
    //verify urs signatue
    G1 A1;
    GT E1,E2,E3;
    ...
    A1=pfc->mult(pp.urs_pub.Y[0],purch_info.derive.s);
    E1=pfc->pairing(purch_info.derive.sigma1,A1);

    E2=pfc->pairing(purch_info.derive.sigma2,g);
    E2=pfc->power(E2,c);
    ...
    for(int i=0;i<DISCLOSE_NUM;i++)
    {
        B1=pfc->mult(pp.urs_pub.Y[purch_info.diclose.index[i]],purch_info.diclose.attr[i]);
        SUM=SUM+B1;
    }
    E3=pfc->pairing(purch_info.derive.sigma1,SUM);
    E3=pfc->power(E3,c);
    E1=E1*E3;
    if(E1 != E2) return 1;

    G2 A2=pp.urs_pub.Y_[0];
    for(int i=0;i<DISCLOSE_NUM;i++)
    {
        A2=A2+pp.urs_pub.Y_[purch_info.diclose.index[i]];
    }
    E1=pfc->pairing(A2,purch_info.derive.sigma1_);
    E2=pfc->pairing(g_,purch_info.derive.sigma2_);
    if(E1 != E2) return 2;

    //verify pok3
    A1=pfc->mult(pp.urs_pub.Y[0],purch_info.pok3.su);
    E1=pfc->pairing(purch_info.derive.sigma1,A1);
    E2=pfc->power(purch_info.pok3.D,purch_info.pok3.c3);

    GT RD=E1*E2;

    G2 RPsu=pfc->mult(seller_key.pub_key.Y_[0],purch_info.pok3.su);
    A2=pfc->mult(seller_key.pub_key.Y_[1],purch_info.pok3.sd);
    RPsu=RPsu+A2;
    A2=pfc->mult(seller_key.pub_key.Y_[2],purch_info.pok3.sr);
    RPsu=RPsu+A2;
    A2=pfc->mult(purch_info.Psu,purch_info.pok3.c3);
    RPsu=RPsu+A2;
...
int EPET::ObtainTick_Receive(SELLER_PUB_KEY &sell_pub, USER_KEY &user_key, TICK_PRI &tick_pri, TICKET &tick)
{
    ...
    GT E1,E2;
    E1=pfc->pairing(tick.T2,g);
    G1 A1,SUM;
    SUM=sell_pub.X_;
    A1=pfc->mult(sell_pub.Y[0],user_key.usk);
    SUM=SUM+A1;
    A1= pfc->mult(sell_pub.Y[1],tick_pri.dsid);
    SUM=SUM+A1;
    A1=pfc->mult(sell_pub.Y[2],tick_pri.dsrnd);
    SUM=SUM+A1;
    A1=pfc->mult(sell_pub.Y[3],tick.VP_t);
    SUM=SUM+A1;
    E2=pfc->pairing(tick.T1,SUM);
    if(E1 !=E2) return 1;

    return 0;
}
//ShowTickets----ValidTickets process
int EPET::ShowTick(SELLER_PUB_KEY &sell_pub, TICK_PRI &tick_pri, TICKET &tick, USER_KEY &user_key, USER_SHOW_INFO &show_info, Big &nounce)
{
    ...
    B1=pfc->mult(sell_pub.Y[0],tick_pri.dsrnd);
    show_info.derive.T3=pfc->pairing(show_info.derive.T1,B1);
    pfc->start_hash();
    ...
    R1=R1+B1;
    B1=pfc->mult(sell_pub.Y[0],alfar);
    R2=pfc->pairing(show_info.derive.T1,B1);
    pfc->start_hash();
    pfc->add_to_hash(show_info.derive.T1_);
    pfc->add_to_hash(show_info.derive.T3);
    pfc->add_to_hash(R1);
...
int EPET::ValidTick(SELLER_PUB_KEY &sell_pub, USER_SHOW_INFO &show_info,BLAME_INFO &blame_info,Big &nounce)
{
    //verify urs
    GT E1,E2,E3;
    G1 A1;
    ...
    pfc->add_to_hash(nounce);
    Big c=pfc->finish_hash_to_group();
    A1= pfc->mult(sell_pub.Y[0],show_info.derive.s);
    E1=pfc->pairing(show_info.derive.T1,A1);

    E2= pfc->pairing(show_info.derive.T2,g);
    E2=pfc->power(E2,c);
    E2= E2*show_info.derive.T3;

    B1=sell_pub.X_+show_info.derive.T1_;
    B2=pfc->mult(sell_pub.Y[1],show_info.dsid);
    B1=B2+B1;
    B2=pfc->mult(sell_pub.Y[3],show_info.VP_t);
    B1=B2+B1;
    E3=pfc->pairing(show_info.derive.T1,B1);
    E3=pfc->power(E3,c);
    E1=E1*E3;
    if(E1 != E2) return 1;
    G2 A2=sell_pub.Y_[0]+sell_pub.Y_[1]+sell_pub.Y_[3];
    E1=pfc->pairing(A2,show_info.derive.T1_);
    E2=pfc->pairing(g_,show_info.derive.T2_);
    if(E1 != E2) return 2;

    //verify pok4
    G1 R1;
    GT R2;
    R1=pfc->mult(g,show_info.pok4.st);
    B1=pfc->mult(sell_pub.Y[2],show_info.pok4.sr);
    B2=pfc->mult(show_info.derive.T1_,show_info.pok4.c4);
    R1=R1+B1+B2;
    R2=pfc->pairing(show_info.derive.T1,sell_pub.Y[0]);
...
{
    G2 upk=pfc->mult(g_,user_key.usk);
